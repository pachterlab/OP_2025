...
```{r}
df <- read.csv('/workspace/alluvial_inputs.csv')
...
color_key <- match(c('BLOOD', 'HEART', 'VENACV', 'SPLEEN', 'SKM-GN', 'SKM-VL', 'WAT', 'BAT', 'LIVER', 'LUNG', 'KIDNEY', 'ADRNL', 'HIPPOC', 'CORTEX', 'HYPOTH', 'SMLINT', 'COLON', 'OVARY', 'TESTES'), unique(df$tissue))
color_list <- c(
...
df <- df %>%
    arrange(match(tissue, 
                  c('BLOOD', 'HEART', 'VENACV', 'SPLEEN', 'SKM-GN', 'SKM-VL', 'WAT', 'BAT', 'LIVER', 'LUNG', 'KIDNEY', 'ADRNL', 'HIPPOC', 'CORTEX', 'HYPOTH', 'SMLINT', 'COLON', 'OVARY', 'TESTES')))
...
```{r}
p <- plot_alluvial(df, graphing_columns = c("Experimenter\n Sample Label", "Transcriptome\n Cluster"),column_weights = "value", sorting_algorithm = 'greedy_WOLF', fixed_column = 'Experimenter\n Sample Label', color_bands=TRUE, color_band_boundary=TRUE,color_band_column = 'matched', color_band_list = c('snow2', 'red3', 'blue3'),
                    color_list = color_list[color_key],include_group_sizes = FALSE, output_plot_path = '/workspace/figures/figure_1c.png'
)
p
```